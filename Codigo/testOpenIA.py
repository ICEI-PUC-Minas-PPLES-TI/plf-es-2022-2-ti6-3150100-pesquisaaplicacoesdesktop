    ...
    for x in myresult:
        ...
        if json['data']['repository'] is not None:
            ...
            if description is not None:
                ...
                question1 = "Describe the domain of this GitHub description"
                response = openai.Completion.create(
                    model="text-davinci-002",
                    prompt=f"{question1}:{description}",
                    temperature=0.6,  # aleatoriedade
                )
                result = response.choices[0].text.strip()
                try:
                    sql = f"INSERT INTO repository_description_openia (repository_id, openia_describe) VALUES ({x[0]}, '{result}')"
                    mycursor.execute(sql)
                    mydb.commit()
                except:
                    print(
                        "Erro ao executar essa query (repository_description_openia): ")
                    print(sql)

                # print(f"Resultado {question1}")
                # print(result)
                # question2 = "Extract list of categories from this text"
                # responseDaResponse = openai.Completion.create(
                #     model="text-davinci-002",
                #     prompt=f"{question2}:{description}",
                #     temperature=0.6,  # aleatoriedade
                # )
                # print(f"Resultado do {question2}")
                # print(responseDaResponse.choices[0].text.strip())
                if len(result) < 50:
                    # descriptionSplitNew = [
